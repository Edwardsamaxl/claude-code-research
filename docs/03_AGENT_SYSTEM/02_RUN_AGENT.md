...
消息流向：
```
runAgent() → yield message → 父 Agent 立即处理
```
...
消息流向：
```
runAgent() → yield message → 后台任务消费 → 存入队列 → 不阻塞父 Agent
```
...
用户 → AgentTool (同步)
         │
         ▼ runAgent()
         ├─ yield stream_event (TTFT)
         ├─ yield assistant (thinking)
         ├─ yield assistant (tool_use: Read)
         ├─ yield user (继续追问)
         ├─ ...更多消息...
         └─ 最终返回结果
```
...
用户 → AgentTool (异步)
         ...
         │   └─ 后台运行 runAgent()
         │        ├─ yield stream_event (丢弃)
         │        ├─ yield assistant (丢弃)
         │        ├─ ...全部后台消费...
         │        └─ 完成后发 task-notification
         │
         ▼ task-notification 推送给用户
         "Agent 已完成：修复了 3 个文件"
```
...
session-<sessionId>/subagents/      # 子 Agent 目录
  agent-<agentId>.jsonl            # 子 Agent 独立记录
```

**作用**：
1. 子 Agent 的消息独立记录，不混入主会话
2. 支持 `--resume` 恢复子 Agent 的执行
3. 每个 Agent 可独立查看自己的消息历史
...
4. 执行循环 (query)
   ...
   - 处理 stream_event, assistant, user 等
